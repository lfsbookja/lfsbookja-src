...
<command>consolelog</command>
の設定方法について説明します。
このスクリプトはキーボードマップ、コンソールフォント、カーネルログレベルを設定します。
非アスキー文字 (例えば著作権、ポンド記号、ユーロ記号など)
を使わず、キーボードが US 配列であるなら、本節は読み飛ばしてください。
<command>console</command>
...
@y
<para>
<command>console</command> スクリプトと <command>consolelog</command>
スクリプトは、設定情報を <filename>/etc/sysconfig/console</filename>
ファイルから読み込みます。
...
を参照してください。
よく分からない場合は
<filename class="directory">/lib/kbd</filename>
ディレクトリを見て、正しいキーマップとスクリーンフォントを探してください。
マニュアルページ <filename>loadkeys(1)</filename> と
<filename>setfont(8)</filename>
...
@y
<para>
この変数は、コンソールに出力されるカーネルメッセージのログレベルを指定するもので
<command>dmesg</command> コマンドにより設定されます。
...
@y
<para>
この変数は <command>loadkeys</command>
プログラムに対する引数を指定します。
このプログラムは <quote>es</quote> などのキーマップをロードします。
この変数がセットされていない場合、ブートスクリプトは <command>loadkeys</command>
...
<para>
この変数は (あまり利用されませんが)
<command>loadkeys</command> プログラムを二度目に呼び出す際の引数を指定します。
普通のキーマップでは十分な設定にならない時の微調整を行うために利用します。
例えばユーロ記号がキーマップの中に含まれておらずこれを付け加える場合には、この変数に対して
<quote>euro2</quote>
を設定します。
</para>
@z
...
一般にこの変数にはフォント名、<quote>-m</quote>、アプリケーションキャラクタマップ
(application character map) を順に指定します。
例えばフォントとして <quote>lat1-16</quote>、アプリケーションキャラクタマップとして
<quote>8859-1</quote>
を指定する場合、この変数には <quote>lat1-16 -m 8859-1</quote>
を設定します。
(これは米国にて適当な設定となります。)
UTF-8 モードの場合、カーネルは UTF-8 キーマップ内の 8 ビットキーコードを変換するためにアプリケーションキャラクタマップを利用します。
...
この変数にて UTF-8 ではないキーマップのエンコーディングが指定されていたら
<command>console</command>
ブートスクリプトは利用可能な UTF-8 キーマップに変換します。
</para>
@z
...
@y
<para>
上の例においては 512 個のグリフを持つ LatArCyrHeb-16
フォントを利用しています。
この場合、フレームバッファを利用していなければ Linux コンソール上に鮮やかな色づけを行うことは出来なくなります。
フレームバッファがない状態で文字フォントを変更することなく色づけを適切に行いたい場合は、以下に示すように
256 個のグリフを持った、この言語に固有のフォントを用いる方法もあります。
</para>
@z
...
@y
<para>
以下の例では ISO-8859-15 から UTF-8 へのキーマップ変換の自動化
(keymap autoconversion)
を指定し、Unicode におけるデッドキー (dead keys) を有効にするものです。
</para>
@z
...
<quote>Ctrl+.、A、E を入力することで &AElig; を得るもの</quote>
があります。
Linux-&linux-version;
...
文字を使ったり、ASCII 文字を二つ使って工夫したりするからです。
しかし UTF-8 モードでは問題になります。
例えばギリシャ語にて <quote>alpha</quote>
の文字の上にアクセント記号を付けたい場合が問題です。
これを解決するには、一つには UTF-8 の利用を諦めることであり、もう一つは
X ウィンドウシステムを使うことで、そのような入力処理の制約を解消することです。
</para>
@z
...
@y
<para>
中国語、日本語、韓国語などを利用する場合
Linux コンソールにはそれらの文字を表示できません。
この言語を利用するユーザーは X ウィンドウシステムを使ってください。
そこで用いるフォントは、必要となるコード範囲の文字を有しており、入力メソッドも用意されています。
(例えば SCIM は数多くの言語入力をサポートしています。)
</para>
@z
...
<para>
<filename>/etc/sysconfig/console</filename>
ファイルは Linux のテキストコンソール上の言語設定を行うだけです。
...
<title>日本語訳情報</title>
<para>
日本の方であれば <quote>日本語106キーボード</quote>
をほぼ間違いなくお使いかと思いますので KEYMAP
変数には <quote>jp106</quote> を設定することになるでしょう。
FONT 変数について訳者は十分な知識がありません。
ここに何を設定すべきか分からない (調べていない)
ため、何も設定しないでいる状態です。
訳者は LFS システム構築後は SSH 接続によりシステムアクセスしており、その場合ここでのフォントの設定がどうであろうと
(おそらく) 無関係であるため、あまり気にせずにいます。
