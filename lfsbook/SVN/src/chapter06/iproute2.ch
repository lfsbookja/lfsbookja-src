...
<para>
<command>ip</command>
コマンドに対するシェルスクリプトラッパー。
<ulink url="http://www.skbuff.net/iputils/"/>
にて提供されている iputils パッケージの
<command>arping</command> プログラムと
<command>rdisk</command> プログラムを利用します。
</para>
@z
...
<para><command>ip addr</command>
はアドレスとその属性を参照し、新しいアドレスの追加、古いアドレスの削除を行います。
</para>

<para>
<command>ip neighbor</command>
は、隣接ルーター (neighbor) の割り当てや属性を参照し、隣接ルーターの項目追加や古いものの削除を行います。
</para>

<para>
<command>ip rule</command>
は、ルーティングポリシー (routing policy)
を参照し、変更を行います。
</para>

<para>
<command>ip route</command> 
は、ルーティングテーブル (routing table)
を参照し、ルーティングルール (routing table rule)
を変更します。
</para>

<para>
<command>ip tunnel</command>
は、IP トンネル (IP tunnel) やその属性を参照し、変更を行います。
</para>

<para>
<command>ip maddr</command>
は、マルチキャストアドレス (multicast address)
やその属性を参照し、変更を行います。
</para>

<para>
<command>ip mroute</command>
は、マルチキャストルーティング (multicast routing)
の設定、変更、削除を行います。
</para>

<para>
<command>ip monitor</command>
...
これはかつての <command>rtstat</command>
プログラムを汎用的に機能充足を図ったプログラムです。
</para>
@z
...
@y
<para>
<filename>/proc/net/rt_acct</filename>
の内容を表示します。
</para>
@z
...
@y
<para>
<command>ip -o</command>
コマンドにより出力される内容を読みやすい形に戻します。
</para>
@z
...
これは QOS (Quality Of Service) と COS (Class Of Service)
を実装するプログラムです。
</para>

<para>
<command>tc qdisc</command>
は、キューイング規則 (queueing discipline) の設定を行います。
</para>

<para>
<command>tc class</command>
は、キューイング規則スケジューリング (queueing discipline scheduling)
に基づくクラスの設定を行います。
</para>

<para>
<command>tc estimator</command>
は、ネットワークフローを見積もります。
</para>

<para>
<command>tc filter</command>
は、QOS/COS パケットのフィルタリング設定を行います。
</para>

<para>
<command>tc policy</command>
は、QOS/COS ポリシーの設定を行います。
</para>
@z
