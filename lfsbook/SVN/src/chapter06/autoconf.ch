...
生成される設定 (<command>configure</command>)
スクリプトは独立して動作します。
つまりこれを実行するにあたっては
...
@y
<para>
C言語の <emphasis>#define</emphasis> 文を configure
...
-->
<para>
ソフトウェアパッケージに対する <filename>configure.in</filename>
ファイルの生成をサポートします。
ディレクトリツリー内のソースファイルを調査して、共通的な可搬性に関わる問題を見出します。
そして <filename>configure.scan</filename>
ファイルを生成して、そのパッケージの
<filename>configure.in</filename> ファイルの雛形として提供します。
</para>
@z
...
ファイルにおいて、かつての古い <command>autoconf</command>
...
@y
<para>
ソフトウェアパッケージにおける <filename>configure.in</filename>
ファイルの記述作成をサポートします。
これはそのパッケージが利用する C プリプロセッサの条件ディレクティブの識別子を出力します。
可搬性を考慮した構築ができている場合は、本プログラムが
<command>configure</command>
スクリプトにおいて何をチェックするべきかを決定してくれます。
また <command>autoscan</command> によって生成された
<filename>configure.in</filename>
