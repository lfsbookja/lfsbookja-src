...
テスト・スイートの実行によりいくつかのチェックが行われ、開発者の意図したとおりにパッケージが正しく動作することを確認していきます。
...
GCC と Glibc におけるテスト・スイートはかなりの時間を要します。
...
(pseudo terminals; PTYs) を使いつくす問題が発生します。
これにより数多くのテストが失敗します。
