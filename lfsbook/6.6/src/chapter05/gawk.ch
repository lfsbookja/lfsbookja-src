%
% This is part of LFSbookja package.
%
% This is a CTIE change file for the original XML source of the LFSbook.
%
% $Author$
% $Rev$
% $Date$
%
@x
<?xml version="1.0" encoding="ISO-8859-1"?>
@y
<?xml version="1.0" encoding="UTF-8"?>
@z

@x
  <indexterm zone="ch-tools-gawk">
    <primary sortas="a-Gawk">Gawk</primary>
    <secondary>tools</secondary>
  </indexterm>
@y
  <indexterm zone="ch-tools-gawk">
    <primary sortas="a-Gawk">Gawk</primary>
    <secondary>ツール</secondary>
  </indexterm>
@z

@x
    <title>Installation of Gawk</title>
@y
    <title>Gawk のインストール</title>
@z

@x
    <para>Prepare Gawk for compilation:</para>
@y
    <para>Gawk をコンパイルするための準備をします。</para>
@z

@x
    <para>Compile the package:</para>
@y
    <para>パッケージをコンパイルします。</para>
@z

% @x
%       <title>The meaning of the configure option:</title>
% @y
%       <title>configure オプションの意味：</title>
% @z
% 
% @x
%           <para>This parameter bypasses the search for mktime in
%           <command>configure</command> and uses the version in Glibc. This is
%           necessary due to a change in GCC that has not been incorporated into
%           this package yet.</para>
% @y
% <para>
% このパラメータは <command>configure</command>
% 実行において mktime の検索を取りやめ Glibc にて提供されているバージョンを用いることを指示します。
% GCC がまだ本パッケージと連携して構築できていないため GCC が変更されることに伴い必要となるものです。
% </para>
% @z

@x
    <para>Compilation is now complete. As discussed earlier, running the test
    suite is not mandatory for the temporary tools here in this chapter. To run
    the Gawk test suite anyway, issue the following command:</para>
@y
<para>
コンパイルが終了しました。
前にも述べたように、この章にて一時的ツールのテストスイートを実行することは必須ではありません。
しかしテストスイートを実行するなら、以下を実行します。
</para>
@z

@x
    <para>To test the results, issue:</para>
@y
<para>コンパイル結果をテストするなら以下を実行します。</para>
@z

@x
    <para>Install the package:</para>
@y
<para>パッケージをインストールします。</para>
@z

@x
    <para>Details on this package are located in
    <xref linkend="contents-gawk" role="."/></para>
@y
<para>
本パッケージの詳細は
<xref linkend="contents-gawk" role=""/>
を参照してください。
</para>
@z

