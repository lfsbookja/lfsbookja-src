...
@y
<para>
匿名アクセスを行う場合は、以下のコマンドを実行します。
(ここでは bash または同等のシェルを利用するものとします。)
</para>
@z
...
@y
<para>
上は LFS ブックの最新リビジョンを<quote>チェックアウト</quote>するものであり、<filename
class="directory">lfs-trunk</filename> というディレクトリに取得します。
取得したローカルコピー (またはワーキングコピーとも呼びます) を更新する場合は、そのディレクトリに <command>cd</command> で移動して以下を実行します。
</para>
@z
