...
@y
<para>
バージョン 4.3 以降の GCC を用いてここでのビルド作業を行うと、出来上がるのは再配置可能なコンパイラ
(relocated compiler) であり、<parameter>--prefix</parameter>
パラメータによって指定されたディレクトリからの起動ファイル (startfiles) の探索が行われないものになります。
しかしここで作り出すのは再配置可能なコンパイラではなく、
<filename class="directory">/tools</filename>
ディレクトリにある起動ファイルは
<filename class="directory">/tools</filename>
ディレクトリ内のライブラリにリンクされたコンパイラを作り出すことが必要であるため、以下のパッチを適用します。
...
@y
<para>
通常の利用環境において GCC が提供する <command>fixincludes</command>
スクリプトは、根本的に不備のあるヘッダファイルを修正する目的で利用されます。
しかしこの時点で GCC-&gcc-version; と Glibc-&glibc-version; 
を既にインストールしており、それぞれのヘッダファイルは修正する必要がないことが分かっています。
つまり <command>fixincludes</command> スクリプトを利用する必要がありません。
もし実行してしまうと、ホストシステムに既に存在していたヘッダファイルが修正され、それが GCC
のプライベートなディレクトリへとインストールされることになり、ビルド環境を壊してしまうことになります。
...
ここで実現したいのは、ブートストラップビルドとまったく同じようにしてコンパイラをビルドしたいものです。
...
<filename class="directory">/tools</filename>
ディレクトリにインストールしたものを用いるようにします。
同時に GCC が探索するインクルードファイルのパスから
<filename class="directory">/usr/include</filename>
を取り除きます。
インストールの後にスペックファイルを調整する方法もありますが、今ここでこのようにするのは
GCC の実際のビルドにおいて新しいダイナミックリンカを用いるようにするためです。
...
まず <filename class="directory">gcc/config</filename> ディレクトリには
<filename>linux.h</filename>、
<filename>linux64.h</filename>、
<filename>sysv4.h</filename>
といったファイルのいずれかがあるはずです。
それらが存在したら、ファイル名称の末尾に <quote>.orig</quote>
をつけたファイルとしてコピーします。
そして一つめの sed
コマンドでは、そのファイル内にある
<quote>/lib/ld</quote>、
<quote>/lib64/ld</quote>、
<quote>/lib32/ld</quote>
という記述部分の頭に <quote>/tools</quote> を付与します。
また二つめの sed
コマンドによってハードコーディングされている <quote>/usr</quote>
という部分を書き換えます。
そしてここで加えるべき定義文をファイルの末尾に追加し、検索パスと startfile プリフィックスを変更します。
最後に <command>touch</command>
によってコピーしたファイルのタイムスタンプを更新します。
<command>cp -u</command>
...
<para>
このオプションはあらゆる状況において
C++ ライブラリに対するロケールモデルが正しく設定されるようにします。
configure スクリプト実行時に <emphasis>de_DE</emphasis>
ロケールがインストール済みであることが分かれば、正しい GNU ロケールモデルが設定されます。
しかし <emphasis>de_DE</emphasis>
ロケールがインストールされていなかったら、誤った汎用ロケールモデルが設定されてしまうため、アプリケーションバイナリインターフェース
(Application Binary Interface; ABI)
とは非互換の C++ ライブラリが生成されてしまう可能性があります。
</para>
@z
...
@y
<para>
このオプションは <function>atexit</function>
を使用せず <function>__cxa_atexit</function>
の使用を有効にします。
これによりローカルなスタティックオブジェクトおよびグローバルオブジェクトに対する
C++ デストラクタを登録します。
このオプションは、標準に完全準拠したデストラクタ実装のために必要です。
またこれは C++ ABI に影響するものであり C++ 共有ライブラリ、C++
プログラムを作り出し、他の Linux ディストリビューションとの互換性を実現します。
</para>
@z
...
@y
<para>
<filename class="libraryfile">libstdc++</filename>
に対してプリコンパイルヘッダ (pre-compiled header; PCH)
をビルドしないように指示します。
...
しかし LFS のビルドでは、何度もブートストラップを行う必要のない、手堅い(?; solid) コンパイラを作り出します。
</para> 
@z
...
最後にシンボリックリンクを作成します。
プログラムやスクリプトの中には
<command>gcc</command> ではなく <command>cc</command>
を用いるものが結構あります。
シンボリックリンクを作ることで各種のプログラムを汎用的にすることができ、通常
GNU C コンパイラがインストールされていない多くの UNIX システムでも利用できるものになります。
<command>cc</command>
を利用することにすれば、
システム管理者がどの C コンパイラをインストールすべきかを判断する必要がなくなります。
</para>
@z
...
@y
<para>
この時点で、構築したツールチェーンの基本的な (コンパイルやリンクなどの)
...
解決せずにこの先に進まないでください。
<command>cc</command> ではなく <command>gcc</command>
を使って再度健全性検査を行ってみてください。
これで解決したなら <filename class="symlink">/tools/bin/cc</filename>
のシンボリックリンクが正しくないということです。
正しく生成し直してください。
また環境変数 <envar>PATH</envar> が正しいかどうかも確認してください。
<command>echo $PATH</command>
を実行して、実行パスリストの先頭が
<filename class="directory">/tools/bin</filename>
であるかどうか確認します。
<envar>PATH</envar>
が間違っていたなら、実はあなたは <systemitem class="username">lfs</systemitem>
ユーザーでログインしていないのかもしれませんし <xref linkend="ch-tools-settingenviron"
role=""/>
...
<para>
本パッケージの詳細は
<xref linkend="contents-gcc" role=""/>
を参照してください。
</para>
@z
