...
@y
    ...
    最近の GCC は GMP、ISL、MPFR、MPC の各パッケージを必要とします。
    これらのパッケージはホストシステムに含まれていないかもしれないため、以下を実行してビルドの準備をします。
    個々のパッケージを GCC ソースディレクトリの中に伸張 (解凍) し、ディレクトリ名を変更します。
    これは GCC のビルド処理においてそれらを自動的に利用できるようにするためです。
    </para>
...
@y
          <para>
          これらのオプションは順に、十進浮動小数点制御、スレッド処理、libatomic, libgomp, libmpx, libquadmath, libssp, libvtv, C++ 標準ライブラリのサポートをいずれも無効にすることを指示します。
...
@x --enable-multilib, --with-multilib-list=m32,m64,mx32
          <para>LFS now supports a multilib configuration. Enable it for the
          32bit, the 64-bit, and the mixed mode.</para>
@y
          <para>
          LFS はマルチライブラリ設定をサポートするようになりました。
          32 ビット、64 ビット、両者混合の各モードにてサポートを有効にします。
          </para>
@z
