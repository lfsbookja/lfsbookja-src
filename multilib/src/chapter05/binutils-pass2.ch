...
@x "ml_32,ml_x32,ml_all" --with-lib-path=/tools/lib...
          <para>This tells the configure script to specify the library
          search path during the compilation of Binutils, resulting in
...
@y
          <para>
          configure スクリプトに対して Binutils のコンパイル中でのライブラリパスを指定します。
          リンカーに対して <filename class="directory">/tools/lib</filename>,
          <filename class="directory">/tools/lib32</filename>,
          <filename class="directory">/tools/libx32</filename> の各ディレクトリを指定するものです。
          こうすることでリンカーがホスト上のライブラリを検索しないようにします。
          </para>
@z
