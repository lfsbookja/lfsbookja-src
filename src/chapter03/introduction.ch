...
@y
  <para>
  パッケージによっては、リリース tarball に加えて (Git や SVN の) リポジトリスナップショット tarball があって、両者を同じファイル名で提供している場合があります。
  リリース tarball には、該当するリポジトリスナップショットの内容に加えて生成済みファイル（たとえば <command>autoconf</command> が作り出す <command>configure</command> スクリプト）が含まれます。
