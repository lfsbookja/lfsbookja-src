...
@y
        ...
        linkend='ch-tools-kernfs'/> において説明している、カーネル仮想ファイルシステムがマウントされていることを確認してください (<command>findmnt | grep $LFS</command> の実行によって、最低でも <filename
        class="directory">$LFS/dev</filename>, <filename
        class="directory">$LFS/proc</filename>, <filename
        class="directory">$LFS/sys</filename> がマウントされていること)。
