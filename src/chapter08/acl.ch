...
@y
    <para>
    Acl のテストは、アクセス制御がサポートされたファイルシステム上にて実施する必要があります。
...
@y
    <para>
    <filename>test/cp.run</filename> というテストは失敗します。
    これは <application>Coreutils</application> が <application>Acl</application> サポートをともなってビルドされていないからです。
    </para>
@z
