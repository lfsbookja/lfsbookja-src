...
@y
    ...
    <filename>run-builtins</filename> というテストは、出力の479 ～ 480行めが異なるということで、特定のホストディストリビューションでは失敗する場合があります。
    テストの中には <literal>zh_TW.BIG5</literal> と <literal>ja_JP.SJIS</literal> のロケールを必要とするものもあります。
    そのロケールがインストールされていなければ、テストが失敗します。
    </para>
@z
