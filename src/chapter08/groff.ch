...
@y
    <para>
    Groff はデフォルトの用紙サイズを設定する環境変数 <envar>PAGE</envar> を参照します。
...
@y
          ...
          コマンドオプションは <option>-e</option>、<option>-man</option>、<option>-me</option>、<option>-mm</option>、<option>-ms</option>、<option>-p</option>、<option>-s</option> のいずれかです。
          そしてそのオプションを含んだ <command>groff</command> コマンドを表示します。
          </para>
@z
...
@y
          <para>
          Canon CAPSL プリンター (LBP-4 または LBP-8 シリーズのレーザープリンター) に対する <command>groff</command> ドライバープログラム。
          </para>
@z
