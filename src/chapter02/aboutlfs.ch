...
@y
  ...
  ホストディストロがこれとは違う値をデフォルトとしている場合を考慮して行うもので
す。
  </para>
@z
...
@y
  <para>
  umask を 022 に設定しておけば、新たに生成するファイルやディレクトリは、所有者
のみが書き込み可能となり、他ユーザーは読み込みおよび (ディレクトリについての) 検
索についてのみ可能となります。
  (これはシステムコール <ulink role='man' url='&man;open.2'>open(2)</ulink> がこのデフォルトモードを利用する前提としており、新規ファイルのモードは 644、新規ディレクトリは 755 となります。)
  LFS システムにおいて、デフォルト値を緩めすぎればセキュリティモードを生み出すことになり、逆に厳しすぎれば、ビルドや LFS システム利用自体に不都合な問題を引き起
こすことにもなります。
  </para>
@z
...
@y
  ...
  class="username">root</systemitem> ユーザーや別のユーザーでログインした場合)
には、忘れずに確認してください。
  </para>
@z
...
@y
  ...
  <filename>.bash_profile</filename> ファイルはログインプロセスの一部として機能
するためです。
  </para>
@z
