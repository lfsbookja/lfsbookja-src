...
@y
    <para>
    Glibc 内のプログラムの中には、FHS には準拠していない <filename
    class="directory">/var/db</filename> ディレクトリに対して実行データを収容しているものがあります。
    以下のパッチを適用することで、FHS に準拠したディレクトリに実行データを収容するように修正します。
    </para>
@z
