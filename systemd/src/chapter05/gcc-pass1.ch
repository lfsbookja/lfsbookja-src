...
@y
          <para>
          これらのオプションは順に、十進浮動小数点制御、スレッド処理、libatomic, libgomp, libitm, libmudflap,
          libquadmath, libsanitizer, libssp, C++ 標準ライブラリのサポートをいずれも無効にすることを指示します。
...
@y
          <para>
          このオプションは C および C++ コンパイラーのみビルドすることを指示します。
