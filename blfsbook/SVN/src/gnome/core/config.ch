...
@y
<para>
<filename>~/.xinitrc</filename> を新規に生成 (あるいは以下の行を追加) して
<application>GNOME</application> を起動するようにします。
</para>
@z
...
<para>
各種ライブラリが確実に参照されるように
(<systemitem class="username">root</systemitem> ユーザーでログインして)
リンカのキャッシュを更新する以下のコマンドを実行します。
</para>
@z
...
@y
<para>
これ以降は <command>startx</command>
を実行すれば <application>GNOME</application>
を起動することができます。
</para>
@z
