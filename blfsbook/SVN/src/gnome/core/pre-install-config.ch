...
@y
    ...
    バージョン番号を持たないこのような名称を用いていく場合は、<systemitem class='username'>root</systemitem> ユーザーになって以下のコマンドを実行してください。
    </para>
@z
...
@y
  <para>
  上でシンボリックリンクを作成した場合は、これ以降の手順説明において <filename
  class='directory'>/opt/gnome-&gnome-version;&gnome-minor-version;</filename> とある箇所をすべて <filename
  class='directory'>/opt/gnome</filename> に置き換えてください。
  </para>
@z
...
@y
  <para>
  <filename>/etc/ld.so.conf</filename> ファイルに対し、以下の記述を加えます。
  </para>
@z
...
@y
  <para>
  <filename>/etc/man_db.conf</filename> ファイルに対し、以下の記述を加えます。
  </para>
@z
...
@y
    <para>
    各種のライブラリをインストールした直後には、リンカーによるライブラリキャッシュを更新するために、<systemitem
    class='username'>root</systemitem> ユーザーになって <command>ldconfig</command> を忘れずに実行してください。
    </para>
@z
