...
@x
  ...
  found in Chapter 19 &ndash; <xref linkend="openssh"/>.  Note that if you only
...
@y
  <para>
  <command>ssh</command> クライアントは、<command>telnet</command> をセキュアにした代替ソフトウェアです。
  これをインストールする場合は、第19章 &ndash; <xref linkend="openssh"/> に示す手順に従ってください。
  クライアントプログラムだけを必要とする場合、サーバープログラムを起動する必要はなく、また初期起動スクリプトやシンボリックリンクも導入する必要はありません。
  このことから、サーバープログラムを起動するのは、それを本当に必要とする時だけにします。
  (もし必要なのかどうなのか不明である場合は、おそらく必要ではないのです！)
  </para>
@z
