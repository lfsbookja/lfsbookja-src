...
@y
          <para>
          Linux From Scratch の systemd チームは独自に systemd ソースの tarball を生成しています。
          その理由としては、man ページとドキュメントをあらかじめ生成しておくためであり、また <application>systemd</application> の git repo から入手したソースでは不要なファイルがインストールされてしまうのでこれを回避するためです。
          Linux From Scratch の systemd チームが生成したこのバージョンを利用するようにしてください。
          </para>
@z
